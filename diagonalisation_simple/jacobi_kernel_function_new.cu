...
void Jacobi_parallel_vec_rot_shared(floatType *d_V, floatType *d_c, floatType *d_s, int *d_P, int *d_Q, int n){
	...
	/* For rows of eigenvectors */
	floatType Vpi, Vqi;
	
	Vpi = V_p[i];
	Vqi = V_q[i];
	
	V_glob[p*n + i] = c*Vpi - s*Vqi;
	V_glob[q*n + i] = c*Vqi + s*Vpi;
	
	/* For columns of eigenvectors */
	//~ floatType Viq, Vip;
...
double jacobi_kernels_parallel(floatType *d_A, floatType *d_W, int m, int batchSize){
	...
	cudaEvent_t start_L, stop_L;
	cudaEventCreate(&start_L);
	cudaEventCreate(&stop_L);
	float time_1, time_2, time_3, time_L;
	time_1 = 0;
	time_2 = 0;
	time_3 = 0;
	...
	for (int iter=0; iter<maxIter; iter++){
		...
		for (int j=0; j<m-1; j++){
			cudaEventRecord(start_L, 0);
			sym_Schur2_all<<<nBlocks, nThreads>>>(d_A, d_c, d_s, d_P, d_Q, m);
			cudaEventRecord(stop_L, 0);
			cudaEventSynchronize(stop_L);
			cudaEventElapsedTime(&time_L, start_L, stop_L);
			time_1 += time_L; time_L=0;
	...
	std::cout << time_1*1e-3 << std::endl;
	std::cout << time_2*1e-3 << std::endl;
	std::cout << time_3*1e-3 << std::endl;
	std::cout << (time_1 + time_2 + time_3)*1e-3 << "\n" << std::endl;
